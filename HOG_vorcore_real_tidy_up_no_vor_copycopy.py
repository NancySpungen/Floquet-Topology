...
delta_t = (T/11)  #double check from notes delta_t = (T/11); J_a_resonance = 2*(np.pi/4)/delta_t 
J_a_resonance = 2*(np.pi/4)/delta_t
J_a = J_a_resonance
...
#%% plot t 
#t_test = np.linspace(0, 1, 100) # function of J_a need to be adjusted as in that t can be out of range of one period(0,T)
...
#    eigvec_U_asc_T_list = []
    ...
    for i in range (len(k_x)):
        ...
        Ufull_list.append(U_full_discretisation(k_x[i], k_y, 200)) # problem identified: time discretisation
    #    print(i)
#        print(Ufull_matrix_asc_list)
...
#   
#%%
def H2NyNx_PBC_version2(N_y, N_x, t, a_0): # time dependent, fine, reak basis but PBC
...
#%% This is main step from H_2x2 [bulk] in k basis to [bulk] in real matrix 
...
#%%
# this are stagewise pbc, only for easier checking spectrum coherency 
# when we finally use U_no vor, we only use H_OBC from H_PBC(t) (--time dependent), not from H_6
...
# This function i used for H2NxNy_xyOBC, hence for no vor
...
#%%
# this is used when no vor 
def H2NyNx_xyOBC(N_y, N_x, t, a_0): #time dependent, fine
...
#%% This is just a mask used by t vor
