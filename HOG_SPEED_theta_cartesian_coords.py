...
#%%
#A = [[1,2,3], [1,2,3], [1,2,3]]
#sites = np.asarray(A) # make a clear copy? 
#i = sites[:, 0] # first column
#j = sites[:, 1] # second column
#s = sites[:, 2] # third column

#%% This cell, include xy OBC condition, only changed func as [H2NyNx_xyOBC] at 3.18. - details see notes inside the cell 
def H2NyNx_xyOBC_time_vor_core(N_y, N_x, t, a_0, x_0, y_0, s_0):  # x_0 and y_0 are the position of time vor core adjustable 
    ...
    for i in range(N_x* N_y):
      ...
      #  delta_x = (x - x_1) % N_x #this gives [the smallest integer which near the (x-x1) and can be divided by N_x]'s difference with (x-x_1)
        ...
        # location of vortex core:
            x_0_real, y_0_real = one_site_to_xy(site_tupil_core, a_0) # function: site_to_plot_xy(sites, a_0)
                                                                                 #  i = sites[:, 0]
                                                                                 #  j = sites[:, 1]
                                                                                 #  s = sites[:, 2]
...
#-----------------------------------------------------------    
            vortex_theta = np.arctan2(y_arv_real, x_arv_real)
...
H_TEST_time_vortex =  H2NyNx_xyOBC_time_vor_core(5, 5, T/12, 1, 2, 2, 2)
    # func: H2NyNx_xyOBC_time_vor_core(N_y, N_x, t, a_0, x_0, y_0, s_0)
...
def U_full_time_vor_core(N_x, N_y, num_time_stages, a_0, x_0, y_0, s_0): # which means along lhs to rhs, from (1) earlier/smaller t to (6) later/larger t
...
#    time_stage = num_time_stages
    ...
    for i in range (num_time_stages):
        ...
        H_i = H2NyNx_xyOBC_time_vor_core (N_y, N_x, t_i, a_0, x_0, y_0, s_0)
...
def U_full_time_vor_midpoint(N_x, N_y, num_time_stages, a_0):
    global_journal.enter("Calculating the U full time vortex midpoint...")
    x_mid = (1+ N_x)/2
    y_mid = (1+ N_y)/2
    s_mid = (1 + 2)/2
    U_full_for_mid_point = U_full_time_vor_core(N_x, N_y, num_time_stages, a_0, x_mid, y_mid, s_mid)
...
#%%
a = np.array([[1,2,3], [2,3,4], [2,2,3]])
x = a[2,:] # 2 label third 'row'
print(x)
...
#%% ###need to be changed !!
#plot fig 4b 
def Rho_all_sites_epsilon_vor(N_x, N_y, num_time_stages, a_0 , sigma, epsilon_specify, list_of_sites_needed=None, n_alpha=2):
...
#Plaq_dict_UC8X8_vor_mid_offset_3 = Rho_all_sites_epsilon_vor(8, 8, 50, 1, 0.01)
#print('offset 0.3', Plaq_dict_UC8X8_vor_mid_offset_3)
#%% FIG 4B CALCULATION
 #VORTEX, PLAQUETTE 
#H2NyNx_xyOBC_time_vor_core(N_y, N_x, t, a_0, x_0, y_0, s_0)
Plaq_dict_UC8X8_vor_mid = Rho_all_sites_epsilon_vor(8, 8, 50, 1, 0.28, np.pi) #offset need to be setted correctly THIS IS 1ST FUNC CALLED; Fig 4b - color intensity prop to rho sum, sites used for corresponding label in real space
#                                                   N_x, N_y, num_time_stages, a_0 , sigma, epsilon_specify
...
Rho, Epsilon = Rho_sites_all_epsilons(8, 8, t_sampling, 1, list_of_sites_around_mid_8) # it seems the more time sampled, the larger the bulk structure(that range of Epsilon)'s Rho 2ND FUNC CALLED
